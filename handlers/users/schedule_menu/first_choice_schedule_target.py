...
from data.messages import ENTER_LECTURER_TARGET_NAME_MESSAGE
from data.messages import ENTER_STUDENT_TARGET_NAME_MESSAGE
...
@dp.message_handler(
    lambda m: m.text in [STUDENT_TARGET_RKB_BUTTON_MESSAGE, LECTURER_TARGET_RKB_BUTTON_MESSAGE],
...
async def first_choice_schedule_target(message: types.Message, state: FSMContext) -> None:
    ...
    if text == STUDENT_TARGET_RKB_BUTTON_MESSAGE:
