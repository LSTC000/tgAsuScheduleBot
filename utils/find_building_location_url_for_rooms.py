...
def find_building_location_url_for_rooms(room: str) -> str:
    '''
    ...
    if 'сок' in lower_room:
        return f'<a href="{BUILDINGS_LOCATION_URL_DICT["сок"]}" title="корпус"><b>{room}</b></a>'

    if 'лыж' in lower_room:
        return f'<a href="{BUILDINGS_LOCATION_URL_DICT["лыж.база"]}" title="корпус"><b>{room}</b></a>'

    split_lower_room = lower_room.split()

    for key in list(BUILDINGS_LOCATION_URL_DICT.keys())[:-2]:
        if key in split_lower_room:
            return f'<a href="{BUILDINGS_LOCATION_URL_DICT[key]}" title="корпус"><b>{room}</b></a>'
